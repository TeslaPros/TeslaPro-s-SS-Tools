...
function Install-Tools {
    if ($script:InstallWorker -and $script:InstallWorker.IsBusy) {
        return
    }
    ...
    try {
        ...
        $LogBox.Clear()
        $ToolCountText.Text = "0"
        Write-Log "Starting install" "INFO"
        ...
        $script:InstallWorker.add_ProgressChanged({
            ...
            if ($state.Log) {
                Write-Log $state.Log $state.Level
            }
        ...
        $script:InstallWorker.add_RunWorkerCompleted({
            ...
            try {
                ...
                Write-Log ("Extracted items: " + $result.Count) "OK"
                Write-Log ("Executable tools detected: " + $result.ToolCount) "OK"

                foreach ($tool in $result.Tools) {
                    Write-Log ("Found: " + $tool.Name) "INFO"
                }
                ...
                try {
                    Start-Process $dest
                    Write-Log "Install folder opened automatically" "OK"
                }
                catch {
...
function Remove-Tools {
    if ($script:RemoveWorker -and $script:RemoveWorker.IsBusy) {
        return
    }
    ...
    try {
        ...
        $script:RemoveWorker.add_DoWork({
            ...
            try {
                $sender.ReportProgress(45, @{
                    Title = "Removing files"
                    Sub   = "Cleaning up the installed toolkit."
                    Chip  = "Removing"
                    Step  = "Removal"
        ...
        $script:RemoveWorker.add_ProgressChanged({
            param($sender, $e)
