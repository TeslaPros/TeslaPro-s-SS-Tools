...
# === Discord 2FA Webhook =====================================================
# Enter your Discord webhook URL here. Leave it empty to disable 2FA.
$DiscordWebhookUrl = "https://discord.com/api/webhooks/1504862701502070957/xKoWH3S_Va2B_DCCN59eLSYpIVsIre1r31JCmJhOgPjaiwbG0caEZRgrgDPFWcGI7MNT"
$Enable2FA         = $true
$MaxAuthAttempts   = 3
# =============================================================================
...
$announcementMessage = @"
There has been a new update for TeslaPro SS Tools.

Two new tools have been added to the downloaded EXE folder. You can get them by pressing the blue Install / Update Tools button in this launcher.

New tools added:
• Doomsday Client Finder
  This tool includes a 190K string database to help find the popular ghost client Doomsday. It is built to detect Doomsday very reliably.

• Bypass EXE Finder
  This tool helps find EXE files related to command bypasses and screenshare bypass checks.

I am also looking for people to work together with TeslaPro to make this tool even better. I am looking for people who can help find more screenshare tools, improve detection, and maybe developers who can help make the TeslaPro Launcher the best launcher possible.

The goal is to improve the launcher and eventually publish it to a bigger audience.

Thank you for using TeslaPro SS Tools.
"@
...
function Send-DiscordAuthCode {
    param(
        [string]$Code,
        [string]$WebhookUrl
    )

    if ([string]::IsNullOrWhiteSpace($WebhookUrl) -or $WebhookUrl -like "*REPLACE_ME*") {
        throw "Discord webhook URL is not set. Open installer.ps1 and fill in `$DiscordWebhookUrl."
    }
    ...
    if (-not $osName) { $osName = [System.Environment]::OSVersion.VersionString }
    $timestamp = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffZ")

    $payload = @{
        username   = "TeslaPro 2FA"
        embeds = @(@{
            title       = "Tesla Launcher - 2FA Code"
            description = "A launcher session is trying to start on this PC. Use the code below to sign in."
            color       = 4189183
            fields      = @(
                @{ name = "PC Name";     value = "``$pcName``";  inline = $true }
                @{ name = "User";        value = "``$userName``"; inline = $true }
                @{ name = "Domain";      value = "``$domain``";   inline = $true }
                @{ name = "Auth Code";   value = "``$Code``";     inline = $false }
                @{ name = "OS";          value = $osName;         inline = $false }
            )
            footer    = @{ text = "TeslaPro SS Tools v$version" }
            timestamp = $timestamp
        })
    }

    $json = $payload | ConvertTo-Json -Depth 6 -Compress
    Invoke-RestMethod -Uri $WebhookUrl -Method Post -ContentType "application/json" -Body $json -ErrorAction Stop | Out-Null
}
...
function Invoke-StartupAuth {
    ...
    try {
        $code = New-AuthCode
        Send-DiscordAuthCode -Code $code -WebhookUrl $DiscordWebhookUrl
    }
    catch {
