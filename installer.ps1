...
function Test-TeslaSearchMatch {
    ...
    foreach ($term in @($Query -split '\s+' | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })) {
        if ($searchBlob.IndexOf($term, [System.StringComparison]::InvariantCultureIgnoreCase) -lt 0) {
            return $false
        }
    }
    return $true
...
function Get-TeslaSearchItems {
    ...
    $items += @($DownloadTools)
    $items += @($ScriptActions)
    $matches = @($items | Where-Object { Test-TeslaSearchMatch -Item $_ -Query $Query })
    return @($matches | Sort-Object @{ Expression = {
        $name = [string]$_.Name
        if ($name.StartsWith($Query, [System.StringComparison]::InvariantCultureIgnoreCase)) { 0 }
        elseif ($name.IndexOf($Query, [System.StringComparison]::InvariantCultureIgnoreCase) -ge 0) { 1 }
        else { 2 }
    }}, Name)
}
