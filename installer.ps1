...
function Start-NextDownload {
            ...
            foreach ($target in $jobArg.Targets) {
                ...
                $sender.ReportProgress(0, [pscustomobject]@{
                    Job = $jobArg
                        ...
                        Percent = 0
                        Bytes = 0L
                        Message = ('[{0}/{1}] Resolving' -f $target.Index, $target.Total)
                    })
                ...
                $downloadMessage = if (Test-Path -LiteralPath $resolvedTarget.Path) { ('[{0}/{1}] Cached' -f $target.Index, $target.Total) } else { ('[{0}/{1}] Downloading' -f $target.Index, $target.Total) }
                $sender.ReportProgress(0, [pscustomobject]@{
                    Job = $jobArg
                    Target = $target
                    HasPercent = $false
    ...
    $worker.Add_RunWorkerCompleted({
        param($sender, $e)
        if ($e.Cancelled) {
            $result = [pscustomobject]@{ Job = $script:ActiveDownloadJob; Success = $false; Cancelled = $true; Error = $null; ErrorType = $null; ErrorUrl = $null; ErrorPath = $null; PrimaryPath = $null; IsZip = $false; Directory = $null; LaunchPath = $null }
        }
        elseif ($e.Error) {
            $activeDir = $ToolsRoot
            if ($script:ActiveDownloadJob -and $script:ActiveDownloadJob.Directory) { $activeDir = $script:ActiveDownloadJob.Directory }
            $result = [pscustomobject]@{
                Job = $script:ActiveDownloadJob
                Success = $false
                Cancelled = $false
            ...
            if (-not $result) {
                $activeDir = $ToolsRoot
                if ($script:ActiveDownloadJob -and $script:ActiveDownloadJob.Directory) { $activeDir = $script:ActiveDownloadJob.Directory }
                $result = [pscustomobject]@{ Job = $script:ActiveDownloadJob; Success = $false; Cancelled = $false; Error = 'The download worker finished but did not return a result object.'; ErrorType = 'MissingResult'; ErrorUrl = $null; ErrorPath = $activeDir; PrimaryPath = $null; IsZip = $false; Directory = $activeDir; LaunchPath = $null }
            }
        }
        $jobDone = $result.Job
        if (-not $jobDone) {
        ...
        elseif ($result.Success) {
            ...
            if (-not $jobDone.Batch -and $result.LaunchPath) {
